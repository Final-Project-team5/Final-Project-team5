"""2단계 — LLM 구체화 + 서비스 진입점 (E87 §8 2단계).

★ 배치 — `copy_model/copy_model/visual_prompt_service.py`
   (파일명은 팀 convention 에 맞춰 조정 가능. `visual_prompt_spec.py` 안으로
    합쳐도 되지만, ★ LLM 을 부르는 코드와 ★ 순수 규칙 코드를 갈라 둔다.)
...
#: ★ E91 — ★ 출력 언어를 ★ 영어로 바꿨으므로 ★ c2.
#: ★ ★ E93 B1(A+) — ★ 출력을 ★ 2채널로 나눴으므로 ★ ★ c3.
...
#: 배경 참고에서 ★ LLM 에 보여줄 축. ★ E87 §3-2 — ★ 공유 5축뿐이다.
