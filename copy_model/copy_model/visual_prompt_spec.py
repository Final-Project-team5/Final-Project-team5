"""사용자의 짧은 시각 요구 → `VisualPromptSpec` (프롬프트 구체화 · E87).
...
`BackgroundContext` 와의 관계 (E87 §3-1)
...
class VisualPromptSpec(BaseModel):
    ...
    ★ `avoid` 는 **1차에서 구조화 결과에만 보존**한다 (E87 §11 ①).
...
def is_ascii_text(value: Any) -> bool:
    """★ E91 언어 게이트 — ★ 이 값이 ★ diffusion 에 실어도 되는 ★ 영어인가.
...
# ──────────────────────────────────────────────────────────────────────────
# ★ reference 병합 — ★ deterministic (E87 §3-2)
# ──────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────
# ★ fallback — ★ LLM 없이 규칙만으로 (E87 §6 · 1단계)
# ──────────────────────────────────────────────────────────────────────────
...
#:   ★ CLIP BPE 는 영어 중심이라 ★ 한국어는 같은 뜻에서 ★ 3~6배 토큰을 쓴다 (E90 §4 실측).
...
#: subject_kind 별 기본 대상 표현. ★ E91 — ★ 영어.
_SUBJECT_TREATMENT = {
...
# ──────────────────────────────────────────────────────────────────────────
# ★ 업종별 장면 — ★ E87 §5-3 "업종별로 나누지 않는다" 를 ★ 여기서만 뒤집는다
# ──────────────────────────────────────────────────────────────────────────
#
# ★ 원래 원칙과 ★ 왜 뒤집는가
#
#   E87 §5-3 은 "업종별 템플릿을 만들지 않는다. 업종 차이는 ★ 사용자 prompt 가
...
#   ★ ★ 그건 이번에 고치려는 결함 그 자체다 (E96/E98 · 2026-08-24 실측).
#
...
def fallback_spec(tone: Optional[str] = None,
    ...
    ★ 두 자리에서 쓴다.
      ① LLM 호출이 실패했을 때 (E87 §6 1단계)
      ② ★ 1단계 구현에서 ★ LLM 없이 경로를 먼저 닫을 때

    ★ ★ E91 — ★ `keywords` 를 ★ 더 이상 문자열에 ★ 붙이지 않는다.
      ★ keywords 는 ★ 사용자가 쓴 ★ 한국어다 ("수분" · "저자극").
      ★ fallback 에는 ★ 번역기가 없다 — ★ 그대로 붙이면 ★ 한국어가 ★ CLIP 으로 간다.
      ★ ★ E90 §4 실측 기준 ★ 한국어 25 tok vs 영어 4 tok — ★ 예산을 그것만으로 태운다.
