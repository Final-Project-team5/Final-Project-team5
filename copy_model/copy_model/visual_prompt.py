"""`VisualPromptSpec` → `visual_prompt` 문자열 (★ deterministic builder · E87 §2-1 · ★ E91 개정).
...
★ ★ 그래서 ★ 카테고리 품질 템플릿(`PROMPT_TEMPLATES`)을 ★ 여기에 두지 않는다.
   ★ 복제하면 ★ poster_model/pipeline/config.py 와 ★ 두 곳이 된다 (E87 §2-1).
...
★ ① ★ 문자 상한(`MAX_CHARS`) 을 ★ 최종 방어선으로 쓰지 않는다
     ...
     ★ ★ 실제 예산 집행은 ★ poster_model 이 ★ 진짜 tokenizer 로 한다
        (`poster_model/prompt_budget.py` · E91 §4).
...
★ `avoid` 는 ★ 여전히 ★ 넣지 않는다 — ★ 1차에서는 ★ 구조화 결과에만 보존한다.
  ★ positive prompt 에 `without ...` 을 억지로 넣지 않는다 (E87 §11 ①).
"""
...
#: builder 판번호. ★ E91 에서 ★ v2 로 올린다 (조립 규칙이 바뀌었다).
VISUAL_PROMPT_BUILDER_VERSION = "v2"

# ──────────────────────────────────────────────────────────────────────────
# ★ 보존 우선순위 (E91 §9 · ★ 사용자 확정안)
# ──────────────────────────────────────────────────────────────────────────
