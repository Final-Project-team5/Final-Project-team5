...
# ---------------------------------------------------------------- 서비스형 전용
#
# 아래 두 값은 subject_kind="service"에만 적용된다. 제품형과 나누는 이유는
# **보존해야 할 원본 제품이 없다**는 것 하나다. 제품형 값은 건드리지 않는다.

# 서비스형 시안 해상도.
#
# SD1.5의 학습 해상도는 512다. 768로 뽑으면 학습 분포를 벗어나 구도가 무너진다
# — 원근이 어긋나고 사물이 반복되며 가구 다리가 늘어진다. 제품형은 누끼 이미지가
# 구도를 잡아주지만 text2img인 서비스형은 그 앵커가 없어 그대로 드러난다.
#
# 실측(2026-08-26, 학원 배경 · 동일 프롬프트):
#
#   768   원근 어긋남 · 책상 다리 왜곡 · 사물 반복      draft 3.4초
#   512   원근 자연스러움 · 심도감 · 왜곡 크게 감소 ★   draft 1.7초
#
# refine이 어차피 1024로 올려 그리므로 시안이 작아도 최종 해상도는 같다.
SERVICE_DRAFT_SIZE = 512

# 서비스형 refine 재해석 강도.
#
# REFINE_STRENGTH=0.35는 제품 사진이 변형되지 않게 원본을 65% 붙잡아 두는 값이다.
# 서비스형에는 지킬 제품이 없는데 같은 상한을 쓰고 있어서, refine이 해상도만
# 올리고 시안의 어색함은 그대로 남겼다.
#
# 실측(같은 시안 · strength만 변경):
#
#   0.35   벽 게시물이 뭉개짐 · 재질과 색감이 밋밋함
#   0.75   게시물이 인쇄물처럼 살아나고 색감·재질이 또렷해짐 ★
#
# 구도는 0.75에서도 유지된다(칠판·게시판·책상 배열 위치가 그대로였다).
# img2img가 잠재 공간에서 원본 형태를 붙잡기 때문이다. 시안 3장 중 하나를
# 고르게 하는 흐름이라 이 일치는 계약에 가깝고, 깨졌다면 값을 올릴 수 없었다.
#
# 비용은 실효 스텝이 10→22로 늘어나는 것이다(REFINE_STEPS × strength).
SERVICE_REFINE_STRENGTH = 0.75
