...
번들 폰트 중 유일하게 한자까지 전부 지원합니다. 다만 픽셀(비트맵 스타일) 폰트라
용도가 제한적입니다.
...
사실과 다르니 참고 바랍니다. 다만 한글이 완성형 수준이라 accent 등급으로 둡니다.
...
글리프 수는 20,172개로 많지만 대부분 한자입니다. 한글은 완성형 2,350자 정확히 지원.
...
- **사용 가능 (11,172자 전수)** — 현대 한글로 표현 가능한 모든 음절 지원. 제한 없이 사용
- **부분 (완성형 2,350~2,781자)** — 1987년 KS X 1001 표준이 담은 "실무에서 쓰이는
  한글"만 지원. 일상 문장의 99.9%는 커버되지만, `쉐`(쉐이크·쉐프·쉐딩),
  `똠`(똠얌) 같은 외래어 표기가 빠짐

**완성형 폰트도 탈락시키지 않고 accent 등급으로 사용합니다.** 짧은 헤드라인에는
...
| accent | Black Han Sans, Galmuri11, HS활공명조, HS유지체, 재민체 3.0, KERIS 케듀 Line, 나눔휴먼 EL/Regular | 완성형 한정 또는 용도 제한 — 폴백 필수 |
