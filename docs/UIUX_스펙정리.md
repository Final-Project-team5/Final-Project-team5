...
6. 톤, 강조포인트, 필요 시 사실값 후속질문 1개, 추가 요청 입력
7. 문구 3개 생성 및 규제 검증
...
### 2-4. 챗봇 단계 — 2026-08-21 확정

- 0단계 질문 “제품이 있으신가요, 서비스 위주이신가요?”는 프론트에서 고정 제공한다.
- 이후 챗봇은 `business_type`을 `spec`에 포함해 stateless 방식으로 호출한다.
- 기존 전체 단계 수와 구조를 유지한다: **product 총 7단계, service 총 6단계**다. 0단계는 프론트 고정 단계이며, product의 사진 업로드는 기존 위치를 유지하고 service에는 노출하지 않는다.
- 마지막 구간은 **톤 → 강조포인트 → 필요 시 조건부 후속질문 최대 1개 → 추가요청** 순서다. 톤과 강조포인트는 유지한다.
- 조건부 후속질문은 별도 step이 아니라 **강조포인트 단계의 서브질문**이다. 후속질문 노출 전후로 `total_steps`와 진행률 분모를 늘리지 않는다.
- 강조포인트를 복수 선택하면 사실 확인이 필요한 항목을 우선순위로 평가해 **최상위 1개만** 묻는다. 우선순위는 가격·할인 등 숫자 사실값 > 위치·거리·영업시간 등 위치/시간 사실값이다. 여러 사실을 한 질문에 통합하지 않는다.
- 사실값 트리거가 없으면 후속질문 없이 바로 추가요청으로 이동한다. 후속질문에는 **건너뛰기**를 제공한다.
- 추가요청의 기존 프리셋·키워드 칩은 삭제한다. 추가요청은 선택 사항인 **한 줄 자유입력**으로 받고 응답의 `max_length`를 적용하며, 건너뛰기를 지원한다.
- 과거의 “모든 질문에서 선택지 4개 + 기타” 규칙은 더 이상 전 단계 공통 규칙이 아니다. 기존 선택형 질문과 용도 고정 선택지는 유지하고, 텍스트 질문만 자유입력 UI를 사용한다.
- 2026-08-21 기준 백엔드의 후속질문·응답 필드와 오탐 방지 로직은 PR #100 머지로 구현 완료됐다. FE의 새 필드 기반 UI·상태 연결은 **미구현**이다.
...
### 4-4. Design Planner — 2026-08-21 미구현·범위 재정의 논의 중

- **확인된 현재 상태:** Core는 production 흐름에 연동되지 않았다. 현재 UI 또는 포스터 요청이 Core/Planner 결과를 실제 사용한다고 표현하지 않는다.
- **유지 방향:** 기존 `/generate/drafts` → 사용자의 시안 선택·문구 편집 → `/generate/refine` UX를 유지한다. Planner가 사용자 편집 단계를 대체하거나 문구 위치·제품 위치·typography·grid/zone을 확정하는 구조는 현재 확정 사항이 아니다.
- **논의 중인 역할:** Planner를 디자인 완성 엔진이 아니라 **추천·보조 역할**로 재정의하는 방향을 검토 중이다.
- **초기 목표:** 배경 레퍼런스가 없는 사용자의 `tone`, `keywords`, `request` 같은 추상 입력을 `material`, `lighting`, `texture`, `mood`, `composition` 등 이미지 생성에 유효한 구체 지시와 diffusion prompt로 확장하는 베이직한 기능을 우선 검토한다.
- 배경 이미지가 있는 경로의 `background_context`와 텍스트 경로의 구조를 맞춰 다운스트림이 가능한 한 동일한 형식을 받도록 하는 방안은 기술 협의 대상이다.
- 연동 전까지 팔레트 추천은 현재 포스터 서버 계약과 product `category` 전달 규칙만을 기준으로 설명한다.
- **visual style 논의 중:** 실사/애니메이션/일러스트 등의 스타일을 시안 선택 이후 적용할 수 있는지, 선택 시안의 내용·구도를 유지하면서 스타일만 바꿀 수 있는지 먼저 기술 검증한다. 검증 결과 전에는 챗봇 단계 추가 또는 시안 선택 이후 UI를 확정하지 않는다.
...
### 화면 A — 챗봇 — 백엔드 부분 구현/FE 미구현·확인 필요
...
- 톤과 강조포인트 선택 UI는 유지한다.
- 강조포인트 응답 뒤 서버가 `input_type: "text"`인 조건부 후속질문을 반환하면 같은 강조포인트 단계 안에서 한 줄 입력과 건너뛰기를 표시한다. 진행률은 증가시키지 않는다.
- 조건부 후속질문이 없으면 바로 추가요청으로 이동한다.
- 추가요청 프리셋은 표시하지 않고 한 줄 자유입력, 글자 수 제한, 건너뛰기를 제공한다.
- 질문 UI는 `input_type`, 입력 제한은 `max_length`, 규제 안내는 `regulation`을 기준으로 처리한다. 기존의 `next_step === total_steps` 추론만으로 자유입력 여부를 판단하지 않는다.
- `block`은 재입력 전 진행 불가, `warn`은 안내 후 진행 가능, `null`은 정상 진행이다.
- 2026-08-21 기준 서버 계약·후속질문은 구현 완료됐으나 위 FE 연결은 미구현이다.
...
### 6-2. `/suggest/options` — 2026-08-21 계약

- 요청은 기존처럼 사용자의 답변, 현재 step, 누적 `spec`을 전달하는 stateless 구조다.
- 기존 응답 필드를 유지하고 다음 필드를 추가한다.

| 필드 | 값/형태 | FE 처리 |
| --- | --- | --- |
| `input_type` | `select` 또는 `text` | 누락 시 기존 `select`로 처리; select는 칩, text는 한 줄 입력 |
| `max_length` | 정수 또는 `null` | text 입력의 글자 수 표시·제출 제한에 적용 |
...
- `regulation`의 `block` 응답을 받은 입력은 확정 사실값으로 저장하거나 다음 단계로 넘기지 않는다.
...
| 사실값 강조포인트가 없음 | 후속질문 없이 추가요청으로 이동 | 2026-08-21 확정/백엔드 구현 완료 |
| 사실값 강조포인트가 복수 | 가격·할인 등 숫자 사실값 > 위치·시간 순으로 최상위 1개만 질문 | 2026-08-21 확정/백엔드 구현 완료 |
| 후속질문 노출 | 강조포인트 서브질문으로 표시하고 `total_steps`·진행률 유지 | 2026-08-21 확정/FE 미구현 |
| 후속질문 건너뛰기 | 사실값을 저장하지 않고 추가요청으로 이동 | 2026-08-21 확정/FE 미구현 |
| 자유입력 규제 `block` | 대체 표현 안내, 저장·진행 차단, 같은 자리에서 재입력 | 2026-08-21 확정/FE 미구현 |
| 자유입력 규제 `warn` | 경고 표시 후 저장·진행 허용 | 2026-08-21 확정/FE 미구현 |
| 자유입력 규제 `null` | 정상 저장·진행 | 2026-08-21 확정/FE 미구현 |
| 추가요청 | 프리셋 없이 한 줄 자유입력, `max_length` 적용, 스킵 가능 | 2026-08-21 확정/FE 미구현 |
...
| visual style 선택 기대 | 시안 선택 후 적용 가능성을 먼저 기술 검증하고 결과 전에는 UI 위치를 확정하지 않음 | 2026-08-21 논의 중 |
...
| 항목 | 상태 | 비고 |
| --- | --- | --- |
| Design Planner Core production 연동 | 미구현 | 완료로 표기 금지; 기존 drafts→편집→refine UX 유지 |
| Design Planner 역할 재정의 | 논의 중 | 추천·보조 역할, 이미지 프롬프트 구체화 중심 |
| visual style 변경 | 논의 중/미구현 | 시안 선택 이후 적용 가능성과 결과 유지 수준 기술 검증 선행 |
| 챗봇 새 필드 FE 연동 | 미구현 | `input_type`, `max_length`, `regulation`, 후속질문 서브단계·스킵 처리 |
...
- [ ] product 진행률은 7단계, service 진행률은 6단계를 유지한다.
- [ ] 강조포인트에 사실값 트리거가 있을 때만 같은 단계에서 후속질문 1개가 표시된다.
- [ ] 가격/할인과 위치/시간을 함께 선택하면 가격/할인 질문만 표시된다.
- [ ] 일반 강조포인트는 후속질문 없이 추가요청으로 이동한다.
- [ ] 후속질문과 추가요청 모두 건너뛰기가 동작한다.
- [ ] 추가요청 프리셋은 없고 한 줄 입력에 `max_length`가 적용된다.
- [ ] `input_type: "select"`는 기존 칩, `input_type: "text"`는 한 줄 입력으로 렌더링된다.
- [ ] 자유입력 규제 결과가 block/warn/null 규칙대로 처리된다.
- [ ] visual style UI는 기술 검증 결과 전까지 확정 기능처럼 노출하지 않는다.
